...
if 'OIDC_CLIENT_ID' in os.environ:
    ...
    if 'ODIC_WELL_KNOWN_URL' in os.environ:
        ...
        if 'OIDC_TOKEN_ENDPOINT' not in os.environ:
            raise Exception('Error: OIDC_TOKEN_ENDPOINT is undefined. Required for oidc configuration')
        if 'OIDC_USERINFO_ENDPOINT' not in os.environ:
            raise Exception('Error: OIDC_USERINFO_ENDPOINT is undefined. Required for oidc configuration')
